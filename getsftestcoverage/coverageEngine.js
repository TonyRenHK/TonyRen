...
var buildClassIdToClassDataMap = function () {
	...
	sfdc_client.tooling.sobject('ApexClass').find({NamespacePrefix:'',Status:'Active',Name : { $like : 'ASI%Test%' }},{Id:1,Name:1,Body:1,IsValid:1}).execute(function (error, data) {
	...
	//sfdc_client.query("SELECT Id, Name,Body,IsValid FROM ApexClass WHERE name like 'ASI%' and name like '%Test%' ",{Id:1,Name:1,Body:1,IsValid:1}, function(error, data) {
		if (error) {
			deferred.reject(new Error(error));
		} else {
			console.log(data);
